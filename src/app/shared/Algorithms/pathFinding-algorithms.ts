...
export class PathFinder {
  ...
  updateNeighboursBFS(current: Box, queue: Box[]) {
    let unvisitedNeightbors: Box[] = this.getNeighboursForDjikstra(
      current
  ...
  updateNeighboursDFS(node: Box, queue: Box[]) {
    let neighbors: Box[] = this.getNeighboursForDjikstra(node).filter(
  ...
  updateNeighboursForAstar(current: Box, openSet: Box[], closedSet: Box[]) {
    ...
    for (let neighbor of neighbours) {
      if (!closedSet.includes(neighbor) && !neighbor.isWall) {
        ...
        if (openSet.includes(neighbor)) {
          neighbor.gScore =
            tempGScore < neighbor.gScore ? tempGScore : neighbor.gScore;
          betterPath = true;
        } else {
  ...
  getNeighboursForAstar(node: Box) {
    let row = node.row;
    let col = node.col;
    let neighbors = [];
  ...
  heuristic(a: Box, b: Box) {
    return Math.abs(a.row - b.row) + Math.abs(a.col - b.col);
  }
  removeFromArray(arr: any[], box: Box) {
    for (let i = arr.length - 1; i >= 0; i--) {
      if (arr[i] == box) arr.splice(i, 1);
    }
  }
  ...
  updateUnvisitedNeighboursFordijkstra(node: Box) {
    let neighbors: Box[] = this.getNeighboursForDjikstra(node).filter(
      (neighbor) => !neighbor.isVisited
    );
    for (let neighbor of neighbors) {
  ...
  getNeighboursForDjikstra(node: Box) {
    let row = node.row;
    let col = node.col;
    let neighbors = [];
    if (row > 0) neighbors.push(this.grid[row - 1][col]);
  ...
  sortNodesByDistance(nodeList: Box[]) {
